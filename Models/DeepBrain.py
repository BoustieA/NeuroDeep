...
"""   #TODO vérifier l'initialisation des poids (à priori passe bien dans chaque submodule)
...
dic={"path_weights": './models/checkpoint_24.pth.tar',
 "reset": "all", "last_layer",
 "n_output":2
 "freeze_type":"all","feature_extractor","progressive",
 }
...
#state_dic=torch.load(path_weights, map_location=torch.device('cpu'))
#model = DeepBrain()

#pre_trained_model=torch.load(path_weights, map_location=torch.device('cpu'))
"""
...
model.FT(dic)
"""
iter_=iter(pre_trained_model["state_dict"])
for key in model.state_dict():
    model.state_dict()[key]=next(iter_)
print()
