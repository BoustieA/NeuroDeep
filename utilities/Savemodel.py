...
class SaveBestModel:
    ...
    def __call__(
        ...
        if current_valid_loss < self.best_valid_loss:
            ...
            save_model(epoch, model, optimizer, criterion, scheduler, self.save_parameters)
            
def save_model(epochs, model, optimizer, criterion, scheduler, parameters):
    model_name = parameters.get("model_name","final_model")
    path_records = parameters.get("path_records","Records/trained_model")
    path_save=os.path.join(path_records,model_name+".pth")
    """
    Function to save the trained model to disk.
    """
    print("Saving model...")
    path_save=os.path.join(path_records,model_name)
    torch.save({
                'epoch': epochs,
